...
int read_from(int client_index, struct sockname *users) {  
    ...
    if (users[client_index].username == NULL) {  
        users[client_index].username = malloc(BUF_SIZE + 1);  
        strncpy(users[client_index].username, buf, BUF_SIZE + 1);  
        return 0;  
    }  
      
    char buf_copy[BUF_SIZE + 1];  
    strncpy(buf_copy, users[client_index].username, BUF_SIZE);  
    char colon[2] = ": ";
    strncat(buf_copy, colon, BUF_SIZE - strlen(users[client_index].username));  
    strncat(buf_copy, buf, BUF_SIZE - strlen(users[client_index].username) - 2);  
    buf_copy[strlen(users[client_index].username) + 2 + strlen(buf) + 1] = '\n';  
    buf_copy[strlen(users[client_index].username) + 2 + strlen(buf) + 2] = '\0';  
  

    for (int index = 0; index < MAX_CONNECTIONS; index++) {  
        fd = users[index].sock_fd;  
        if (users[index].sock_fd > -1) {  
            if (num_read == 0 || write(fd, buf_copy, strlen(buf_copy)) != strlen(buf_copy)) {  
                users[client_index].sock_fd = -1;  
                return fd;  
            }  
        }  
